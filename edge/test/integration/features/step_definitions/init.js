...
var environment = config['test_101023'].env;
...
console.log('&test_101023 api: [' +  domains[environment] + ', ' + config['test_101023'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['test_101023'].app, config['test_101023'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['test_101023'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
